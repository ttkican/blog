<template>
    <div class="app-container">
        ...
        <!-- 文章内容 -->
        <md-editor ref="editorRef" v-model="articleForm.articleContent" class="md-container" @on-upload-img="uploadImg"
            :toolbars="toolbars" placeholder="请输入文章内容...">
        ...
        <el-dialog title="发布文章" v-model="addOrUpdate" width="600px" top="0.5vh" append-to-body>
            <el-form ref="articleFormRef" label-width="80px" :model="articleForm" :rules="rules">
                ...
                <el-form-item label="置顶" prop="isTop">
                    <el-switch v-model="articleForm.isTop" active-color="#13ce66" inactive-color="#F4F4F5"
            ...
            if (articleForm.value.id !== undefined) {
                updateArticle(articleForm.value).then(({ data }) => {
                    if (data.flag) {
                        ...
                        tag.delView({ path: `/blog/article/article/${articleForm.value.id}` });
                        router.push({ path: "/blog/article/article-list" });
                    }
                ...
                addArticle(articleForm.value).then(({ data }) => {
                    if (data.flag) {
                        notifySuccess(data.msg);
                        tag.delView({ path: "/blog/article/publish" });
                        router.push({ path: "/blog/article/article-list" });
                    }
                    addOrUpdate.value = false;
                });
    ...
    if (articleId) {
        editArticle(Number(articleId)).then(({ data }) => {
            if (data.flag) {
                ...
                tag.delView({ path: `/blog/article/article/${articleId}` });
                router.push({ path: "/blog/article/article-list" });
            }
        });
    }
